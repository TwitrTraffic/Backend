...
def retrieveAllblrTweets():
    ...
    entries = [dict(id=row[0], tweet=row[1], time=row[2], date=row[3])
...
def getCheckpointLocations(source, destination):
    ...
    for i in json_checkpoints:
        url = 'https://maps.googleapis.com/maps/api/geocode/json?latlng=' + \
            str(i['lat']) + ',' + str(i['lng']) + '&key=' + YOUR_API_KEY
        response = urlopen(url)
        json_obj = json.load(response)
...
def insertRouteIntoDb(source, destination, locations):
    ...
    g.db.execute('insert into places values (?, ?, ?)', [
                 source, destination, locations_string])
    g.db.commit()
...
def getTrafficTweetsForRoute(locations, date, time):
    ...
    for loction in locations:
        cmd = "select tweet,Ttime,Tdate from tweets where tweet like '%" + \
            loction + "%' and Tdate='" + str(date) + "'order by Tdate desc"
